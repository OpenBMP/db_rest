...
public class Upstream {
	...
	public Response getUpstream(@PathParam("ASN") Integer asn) {
	    ...
	    query.append("           SUBSTRING_index(concat(as_path, ' '), ' " + asn + " ', 1), ' '), ' ', -2)) as unsigned) as UpstreamAS,\n");			
		...
		query.append("     group by UpstreamAS\n");
		query.append("     order by null\n");
		*/
		
		query.append("select UpstreamAS,as_name,country,org_name\n");
		...
		query.append("            left join gen_whois_asn w ON (p.UpstreamAS = w.asn)\n");
		query.append("    where UpstreamAS > 0\n");
		query.append("    group by UpstreamAS\n");
		query.append("    order by UpstreamAS\n");
	...
	public Response getUpstreamCount(@PathParam("ASN") Integer asn) {
		...
		StringBuilder query = new StringBuilder();
		query.append(" select UpstreamAS, count(prefix) as Prefixes_Learned,\n");
	    query.append("           as_name,country,org_name\n");			
	    query.append("     from (select distinct hash_id,peer_hash_id,cast(trim(substring_index(concat(SUBSTRING_index(\n");
	    query.append("               concat(as_path, ' '), ' "+asn+" ', 1), ' '), ' ', -2)) as unsigned) as UpstreamAS\n");
	    query.append("            from path_attrs PARTITION \n");
		...
		/*
		query.append("select UpstreamAS, count(distinct prefix, prefix_len) as Prefixes_Learned,\n");
		query.append("              as_name,country,org_name\n");
		...
		*/
		...
		return RestResponse.okWithBody(
					DbUtils.selectPartitions_DbToJson("upstreamASNCount", mysql_ds, query.toString(), 0, 47, includeColumns));
	}
	...
	public Response getUpstreamPeerCount(@PathParam("ASN") Integer asn) {
		...
		StringBuilder query = new StringBuilder();
		query.append("select rtr.ip_address as BMPRouter, bp.peer_addr as PeerAddr, d.UpstreamAS,d.Prefixes_Learned,\n");
		query.append("                   as_name,country,org_name\n");
		query.append("        FROM (select peer_hash_id,UpstreamAS, count(distinct prefix, prefix_len) as Prefixes_Learned\n");
		...
		query.append("    left join gen_whois_asn w ON (d.UpstreamAS = w.asn)\n");
	    
		query.append("    group by rtr.ip_address,bp.peer_addr,d.UpstreamAS\n"); 
		query.append("    order by BMPRouter,PeerAddr,cast(d.UpstreamAS as unsigned)\n");
		...
		return RestResponse.okWithBody(
					DbUtils.selectPartitions_DbToJson("upstreamASNPeerCount", mysql_ds, query.toString(), 0, 47, includeColumns));
	}
